...
ToutiaoLength = Literal["short", "standard", "long"]
ToutiaoCoverStyle = Literal["realistic", "editorial", "tech", "local", "data"]


class ToutiaoPackageRequest(BaseModel):
    topic: str = Field(..., min_length=2, max_length=120)
    facts: str = Field(..., min_length=10, max_length=4000)
    angle: str = Field("", max_length=300)
    audience: str = Field("今日头条普通读者", max_length=120)
    article_style: ToutiaoArticleStyle = "news"
    length: ToutiaoLength = "standard"
    cover_style: ToutiaoCoverStyle = "editorial"
    include_image: bool = True
    client_user_id: Optional[str] = Field(None, max_length=80)

    @field_validator("topic", "facts", "angle", "audience", "client_user_id")
    @classmethod
    def strip_toutiao_text(cls, value: str | None) -> str | None:
        if value is None:
            return None
        return value.strip()
    ...
    @field_validator("client_user_id")
    @classmethod
    def validate_toutiao_client_user_id(cls, value: str | None) -> str | None:
        if not value:
            return None
        if not re.fullmatch(r"[A-Za-z0-9_-]{6,80}", value):
            raise ValueError("client_user_id must be 6-80 chars of letters, numbers, _ or -")
...
class ToutiaoPackageOut(BaseModel):
    best_title: str = Field(..., min_length=1, max_length=80)
    title_options: list[str] = Field(default_factory=list, max_length=5)
    lead: str = Field(..., min_length=1, max_length=240)
    body: str = Field(..., min_length=1, max_length=6000)
    summary_bullets: list[str] = Field(default_factory=list, max_length=6)
    cover_brief: str = Field(..., min_length=1, max_length=300)
    image_prompt: str = Field(..., min_length=1, max_length=1200)
    image_negative_prompt: str = Field("", max_length=500)
    compliance_notes: list[str] = Field(default_factory=list, max_length=8)
    ...
    @classmethod
    def strip_package_text(cls, value: object) -> object:
        if isinstance(value, str):
            return value.strip()
        return value


class ToutiaoPackageResponse(BaseModel):
    package: ToutiaoPackageOut
    image_job: Optional[JobOut] = None
