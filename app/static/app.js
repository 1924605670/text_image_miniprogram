...
async function loadHealth() {
  try {
    ...
    providerTitle.textContent = `${health.provider.image_model} · ${mode} · ${health.retry.attempts} retries`;
    healthBadge.textContent = health.provider.has_api_key ? "服务器已连接" : "缺少 key";
  ...
  card.innerHTML = `
    <div class="history-title">
      ...
      <span class="pill ${job.status}">${statusText(job.status)}</span>
        ...
        <p>${new Date(job.created_at).toLocaleString()} · ${styleText(job.request.style_preset)} · ${job.request.size}</p>
    <div class="row">
  ...
  const meta = `
    <div class="job-meta">
      <div>
        ...
        <p>${styleText(job.request.style_preset)} · ${job.request.size} · ${job.request.quality} · ${job.request.output_format}</p>
      </div>
